<template>
  ...
  <van-row gutter="15" style="padding: 0 15px">
    <van-col v-for="tag in activeIds">
      <van-tag closeable size="small" color="#7232dd" plain @close="doClose(tag)">
        {{ tag }}
      </van-tag>
    </van-col>
  </van-row>
...
const doEditTags = async () => {
  ...
  //console.log(editTags.value.editId);
  // if(editTags.value.editId===undefined){
  //   editTags.value.editId=currentUser.id;
  // }
  const res = await myAxios.post("/user/update", {
    id: editTags.value.editId,
