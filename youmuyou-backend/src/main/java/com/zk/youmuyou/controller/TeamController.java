...
import com.zk.youmuyou.common.DeleteRequest;
...
import com.zk.youmuyou.model.domain.UserTeam;
import com.zk.youmuyou.model.dto.TeamQuery;
import com.zk.youmuyou.model.request.TeamAddRequest;
...
import com.zk.youmuyou.service.TeamService;
import com.zk.youmuyou.service.UserService;
import com.zk.youmuyou.service.UserTeamService;
import lombok.Data;
...
@RestController
@RequestMapping("/team")
@CrossOrigin(origins = {"http://localhost:3000","http://120.78.130.59:80"})
@Slf4j
public class TeamController {
    ...
    public BaseResponse<List<TeamUserVO>> listMyJoinTeams(TeamQuery teamQuery, HttpServletRequest request) {
        ...
        for(int i = 0, len = teamList.size(); i < len; i++){
            if(teamList.get(i).getUserId()==loginUser.getId()){
                teamList.remove(i);
                len--;
                i--;
            }
     ...
     * @return
     */
    @GetMapping("/list/my/private")
    public BaseResponse<List<TeamUserVO>> listMyPrivateTeams(TeamQuery teamQuery, HttpServletRequest request) {
        if (teamQuery == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
